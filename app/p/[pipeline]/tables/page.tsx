...
export default function TablesPage() {
  ...
  const cardClass = (id: string) => {
    if (referencedTables.has(id)) return "border-orange-400 bg-orange-50 ring-2 ring-orange-200";
    return "border-gray-200 bg-white hover:border-gray-300";
  };

  return (
    <main className="mx-auto max-w-6xl px-6 py-8" style={{ minHeight: `calc(100vh - ${TOP_NAV_HEIGHT_PX}px)` }}>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {tables.map((t) => (
          <button
            key={t.id}
            type="button"
            onClick={() => { setSql(`SELECT * FROM ${t.id} LIMIT 50`); runQuery(`SELECT * FROM ${t.id} LIMIT 50`); }}
            className={`rounded-lg border p-4 text-left transition ${cardClass(t.id)}`}
          >
            <div className="flex items-center justify-between">
              <span className="text-sm font-semibold text-gray-800">{t.name}</span>
              <code className="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] text-gray-500">{t.id}</code>
            </div>
            <div className="mt-1 text-xs text-gray-400">
              {t.schema.length} columns · {t.fileCount} file{t.fileCount !== 1 ? "s" : ""}
              {loadedTables.has(t.id) && <> · <span className="text-green-600">loaded</span></>}
            </div>
            <div className="mt-2 flex flex-wrap gap-1">
              {t.schema.map((c) => (
                <span key={c.name} className="rounded bg-gray-50 px-1.5 py-0.5 text-[10px] text-gray-500">{c.name}</span>
              ))}
            </div>
          </button>
        ))}
      ...
      <div className="mt-6 rounded-lg border border-gray-200 bg-white p-5">
        ...
        {result && (
          <div className="mt-3 max-h-[400px] overflow-auto rounded border border-gray-100">

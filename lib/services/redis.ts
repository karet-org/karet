// Shared Redis client (Valkey/Redis via node-redis). One lazily-connected
// client per process, held on `globalThis` so dev-mode HMR reuses the socket.
...
/**
 * The shared client, connected on first use. A failed connect never poisons
 * future calls: `connecting` clears in `finally`, so the next caller retries.
 */
  ...
  state.connecting = (async () => {
    const c: RedisClientType = createClient({
      ...
      socket: {
        ...
        // Backoff capped at 5s, retry forever: the queue is core
        // infrastructure, giving up is never better.
